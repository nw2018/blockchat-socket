...
class RoomManager {
  ...
  getNearbyRooms(location){
    ...
    for(let offset of offsets){
      ...
      let tmpRoom = new Room(crypto.createHash('md5').update(JSON.stringify(tmpLocation)).digest("hex"));
      roomlist.push({location:{lat:location.lat+offset[0],lng:location.lng+offset[1]}})
      if(Math.random()>0.4)
